<?php
	...
	class Day01 extends Day
	{
		...
		{
			if(preg_match_all('/^(\d+)\s+(\d+)$/m', $this->Input, $matches, PREG_PATTERN_ORDER, 0))
			{
				$this->_leftList = $matches[1];
				$this->_rightList = $matches[2];
			}
		}
		...
		{
			sort($this->_leftList);
			sort($this->_rightList);

			$distances = [];
			foreach($this->_leftList as $k => $number)
			{
				$distances[] = abs(($this->_rightList[$k]??0)-$number);
			}
		 ...
		 * The second number in the left list is 4. It appears in the right list once, o the
		...
		public function Part2() : int
		{
			...
			foreach($this->_leftList as $k => $number)
			{
				$occurences = count(array_keys($this->_rightList, $number));
				if($occurences > 0)
				{
					$distances[] = ($number * $occurences);
				}
			}
