<?php
	 ...
	 * still talk about the time Rudolph was saved through molecular synthesis from a single
	 * electron.
	 * 
	...
	class Day02 extends Day
	{
		...
		public function HandleInput()
		{
			...
			{
				$this->_reports[] = preg_split('/\s/', $line);
			}
		}
		...
		{
			$safeReports = 0;
			foreach($this->_reports as $k => $report)
			{
				if($this->_reportIsSafe($report))
				{
					$safeReports++;
				}
			}
		...
		public function Part2()
		{
			...
			foreach($this->_reports as $k => $report)
			{
				...
				{
					$dampeneredReport = $report;
					unset($dampeneredReport[$r]);
					if($this->_reportIsSafe(array_values($dampeneredReport)))
					{
						$safeReports++;
						break;
					}
		...
		private function _reportIsSafe(array $report)
		{
			$reportSortedAsc = $report; sort($reportSortedAsc);
			$reportSortedDesc = $report; rsort($reportSortedDesc);

			if((count(array_diff_assoc($report, $reportSortedAsc)) > 0) && (count(array_diff_assoc($report, $reportSortedDesc)) > 0))
			{
				return false;
			}

			for($k = 1; $k<count($report); $k++)
			{
				$differ = abs($report[$k] - $report[$k-1]);
